...
_PARSER = argparse.ArgumentParser(prog='code-coverage-test-gap.py',
                                  usage='%(prog)s [options]')
_PARSER.add_argument('tstfiles', nargs='+', type=str,
...
_PARSER.add_argument('--open', nargs='?', type=str,
                     help=('open the html page for this file ' +
                           '(default: None)'),
                     default=None)
...
print(_INFO_PREFIX + 'Using temporary directory: ' + _DIR + '\033[0m')
...
filesdir := \\"''' + os.getcwd() + _PROFILE_DIR + '''\\";;\n'''
...
_RUN_GAP = _ARGS.gap_root + 'bin/gap.sh -A -m 1g -T --cover ' + _DIR + '/profile.gz'
...
suffix = ''
if _ARGS.open:
    filename = (_DIR + '/' + os.getcwd().replace('/', '_') + '_'
                + _ARGS.open.replace('/', '_') + '.html')
    p = re.compile('<tr class=\'missed\'><td><a name="line(\d+)">')
...
if exists(filename) and isfile(filename):
    if _ARGS.open:
        filename += suffix
    try:
        webbrowser.get('safari').open('file://' + filename, new=2)
    except Exception:
